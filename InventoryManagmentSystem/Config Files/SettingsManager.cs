...
namespace InventoryManagmentSystem
{
    internal class SettingsManager
    {
        ...
        public void LoadSettings()
        {
            ...
            {
                // Try to copy from your project root
                string defaultFile = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "..", "..", "settings.json");
                if (File.Exists(defaultFile))
                {
                    // Copy the default file to the expected output location
                    File.Copy(defaultFile, _settingsFilePath);
                    // Load the just-copied file
                    string json = File.ReadAllText(_settingsFilePath);
                    _settingsJson = JObject.Parse(json);
                }
                else
                {
                    throw new Exception("File settings.json missing");
                }
            }
        }
